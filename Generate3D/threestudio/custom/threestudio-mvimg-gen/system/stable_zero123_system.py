...
class Zero123Simple(BaseLift3DSystem):
    ...
    def test_step(self, batch, batch_idx):
        ...
        # if not os.path.exists(f"it{self.true_global_step}-test/rgb_images/"):
        #     os.system('mkdir -p ' + f"it{self.true_global_step}-test/rgb_images/")
        self.save_rgb_image(f"it{self.true_global_step}-test/rgb_images/{str(batch['index'][0].item()).zfill(4)}.png", img=out["imgs_final"][0])

...
class StableZero123Guidance(BaseObject):
    ...
    def prepare_embeddings(self, image_path: str) -> None:
        ...
        # pad_width = 40  # Number of pixels to pad on each side
        # rgba = np.pad(rgba, ((pad_width, pad_width), (pad_width, pad_width), (0, 0)), mode='constant', constant_values=255)

        # print(rgba.shape, "rgb shape check....")
        rgba = (
        ...
        # Image.fromarray(np.asarray(rgb*255, dtype=np.uint8)).save('/data/vision/torralba/sduggal/mnt_nfs_data/research/threed_eval/rgb.png')

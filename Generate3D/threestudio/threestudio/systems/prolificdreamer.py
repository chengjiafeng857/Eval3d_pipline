...
class ProlificDreamer(BaseLift3DSystem):
    ...
    def test_step(self, batch, batch_idx):
        out = self(batch)
        # if not os.path.exists(f"it{self.true_global_step}-test/batch_data/"):
        #     os.system('mkdir -p ' + f"it{self.true_global_step}-test/batch_data/")
        #     os.system('mkdir -p ' + f"it{self.true_global_step}-test/rgb_images/")
        #     os.system('mkdir -p ' + f"it{self.true_global_step}-test/normal_world/")
        #     os.system('mkdir -p ' + f"it{self.true_global_step}-test/opacity/")
        #     os.system('mkdir -p ' + f"it{self.true_global_step}-test/image_grid/")
        

        self.save_rgb_image(f"it{self.true_global_step}-test/opacity/{str(batch['index'][0].item()).zfill(4)}.png", img=out["opacity"][0])
        if "comp_rgb" in out: self.save_rgb_image(f"it{self.true_global_step}-test/rgb_images/{str(batch['index'][0].item()).zfill(4)}.png", img=out["comp_rgb"][0])
        if "comp_normal" in out: self.save_normal_map(f"it{self.true_global_step}-test/normal_world/{str(batch['index'][0].item()).zfill(4)}.png", img=out["comp_normal"][0].cpu().numpy())
        import numpy as np
        # np.save(self.get_save_path(f"it{self.true_global_step}-test/normal_world/{str(batch['index'][0].item()).zfill(4)}.npy"), out["comp_normal"][0].cpu().numpy())
    ...
    def on_test_epoch_end(self):
        try:
            ...
            self.save_img_sequence(
                f"it{self.true_global_step}-test",
                f"it{self.true_global_step}-test/normal_world/",
                "(\d+)\.png",
                save_format="mp4",
                fps=30,
                name="test",
